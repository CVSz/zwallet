...
export type BundlerClient = {
  simulate: (op: any) => Promise<{ ok: boolean; reason?: string }>;
...
export class InMemoryBundlerClient implements BundlerClient {
  ...
  async simulate(op: any) { if (String(op.callData).includes('fail')) return { ok: false, reason: 'simulation_failed' }; return { ok: true }; }
...
export class UserOperationService {
  ...
  constructor(private bundler: BundlerClient) {}
